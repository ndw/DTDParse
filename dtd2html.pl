...
sub formatElementFooter {
    ...
    $html .= " by <A HREF=\"http://nwalsh.com/perl/dtdparse/\">";
...
sub writeIndex {
    ...
    my $title     = $root->getAttribute('title');
    my $entfile   = $config{"expanded-entity-index"};
    my $elemfile  = $config{"expanded-element-index"};
	...
	my ($pub) = $root->getAttribute('public-id');
	my ($sys) = $root->getAttribute('system-id');
